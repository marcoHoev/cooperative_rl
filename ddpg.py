...
class DDPG:
    ...
    def save(self, filename_actor, filename_critic, logger=None):
        try:
            self.critic.save_model(filename_critic)
            self.actor.save_model(filename_actor)
        except Exception as e:
            if logger is not None:
                logger.info("Error: " + e.__repr__())
            else:
                print("Error: " + e.__repr__())

    def act(self, obs, explore=False, eps=0.0):
        ...
        if self.discrete_action:
            if explore:
                action = misc.gumbel_softmax(action, hard=True, eps=eps)
            else:
    ...
    def train(self, workers):
        ...
        if self.alg_mode == MADDPG:
            if self.discrete_action:
                target_actions = [misc.onehot_from_logits(worker.target_actor(nobs)) for worker, nobs in
                                  zip(workers, obs_next_n)]
            else:
...
class DEMADDPG:
    def __init__(self, pos, actor, loc_critic, glob_critic, target_actor, target_loc_critic, target_glob_critic, gamma,
                 batch_size, train_mode, discrete_action,
                 alg_mode='DEMADDPG'):

        self.actor = actor
        self.pos = pos
        self.loc_critic = loc_critic
        self.glob_critic = glob_critic
        self.target_actor = target_actor
        self.target_loc_critic = target_loc_critic
        self.target_glob_critic = target_glob_critic
        self.gamma = gamma
        self.batch_size = batch_size
        self.train_mode = train_mode
        self.discrete_action = discrete_action
        ...
        self.target_actor.hard_copy(actor)
        self.target_loc_critic.hard_copy(loc_critic)
        self.target_glob_critic.hard_copy(glob_critic)
    ...
    def load(self, filename_actor, filename_loc_critic, filename_glob_critic):
        try:
            self.loc_critic.load_model(filename_loc_critic)
            self.glob_critic.load_model(filename_glob_critic)
            self.actor.load_model(filename_actor)
        except Exception as e:
            print(e.__repr__)

    def save(self, filename_actor, filename_loc_critic, filename_glob_critic, logger=None):
        try:
            self.loc_critic.save_model(filename_loc_critic)
            self.actor.save_model(filename_actor)
            if self.pos == 0:
                self.glob_critic.save_model(filename_glob_critic)
        except Exception as e:
            if logger is not None:
                logger.info("Error: " + e.__repr__())
            else:
    ...
    def train(self, workers):
        ...
        # Train the local critic network.

        # Get actions in DEMADDPG mode.
        ...
        predicted_q_value = self.target_loc_critic(obs2_concat, target_actions)
        yi = r_batch + ((1 - t_batch) * self.gamma * predicted_q_value).detach()

        obs_concat = s_batch
        action_concat = a_batch
        glob_obs = torch.cat(obs_n, dim=-1)

        predictions = self.loc_critic.train_step(obs_concat, action_concat, yi)

        ep_ave_max_q_value = np.amax(predictions.cpu().detach().numpy())
        # End train local critic model in DEMADDPG

        # Train global critic in DEMADDPG

        if self.discrete_action:
            glob_target_actions = [misc.onehot_from_logits(worker.target_actor(nobs)) for worker, nobs in
                                   zip(workers, obs_next_n)]
        else:
            glob_target_actions = [worker.target_actor(nobs) for worker, nobs in zip(workers, obs_next_n)]

        glob_obs2 = torch.cat(obs_next_n, dim=-1)
        glob_target_actions = torch.cat(glob_target_actions, dim=-1)

        # First get policy and actions (local and global)
        all_pol_acs = []
        if self.discrete_action:
            curr_pol_out = self.actor(s_batch)
            curr_pol_vf_in = misc.gumbel_softmax(curr_pol_out, hard=True)
        ...
        for i, worker, obs in zip(range(len(workers)), workers, obs_n):
            if i == self.pos:
                ...
                # all_pol_acs.append(misc.onehot_from_logits(worker.actor(obs)))
                all_pol_acs.append(misc.gumbel_softmax(worker.actor(obs), hard=True))
            else:
                all_pol_acs.append(worker.actor(obs))
        glob_act = torch.cat(all_pol_acs, dim=-1)

        if self.pos == 0:  # only train global critic once per episode, not for every agent
            glob_predicted_q_value = self.target_glob_critic(glob_obs2, glob_target_actions)
            glob_yi = r_batch + ((1 - t_batch) * self.gamma * glob_predicted_q_value).detach()
            glob_predictions = self.glob_critic.train_step(glob_obs, glob_act, glob_yi, 'global')
            # glob_ep_ave_max_q_value = np.amax(glob_predictions.cpu().detach().numpy())
            self.update_glob_target()

        # Train the actor network.
        self.actor.train_step((self.loc_critic, self.glob_critic), (obs_concat, glob_obs), (act_n_concat, glob_act),
                              curr_pol_out, 'DEMADDPG')

        return ep_ave_max_q_value, True

    def update_glob_target(self):
        self.target_glob_critic.update(self.glob_critic)

    def update_loc_targets(self):
        self.target_actor.update(self.actor)
        self.target_loc_critic.update(self.loc_critic)
