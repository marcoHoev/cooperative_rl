...
def make_env(scenario_name, configuration=None, benchmark=False):
    from multiagent.environment import MultiAgentEnv
    import multiagent.scenarios as scenarios

    # load scenario from script
    scenario = scenarios.load(scenario_name + ".py").Scenario(kwargs=configuration)
    # create world
...
class EnvWrapper:
    def __init__(self, gym_env, actors, saved_episode,
                 configuration=None, update_obs=None,
                 update_reward=None, end_episode=None, benchmark=False):
        self.envs = []
        self.variables = []
        ...
        for _ in range(actors):
            env = make_env(gym_env, configuration, benchmark=self.benchmark)
    ...
    def step(self, actions):
        ...
        for i, action in enumerate(actions):
            ...
            states, rewards, done_, info = self.envs[i].step(action)  # action
            """
            if done_ == True:
    ...
    def render(self, id, title="train.py"):
        self.envs[id].render(title=title)
