...
class Actor(torch.nn.Module):
    ...
    def train_step(self, critic, states, actions, curr_pol_out, mode=None):
        if mode == 'DEMADDPG':  # Two critics for DEMADDPG
            loc_critic, glob_critic = critic
            loc_states, glob_states = states
            loc_actions, glob_actions = actions
            actor_loss = (-loc_critic(loc_states, loc_actions) - glob_critic(glob_states, glob_actions)).mean()
...
class Critic(torch.nn.Module):
    ...
    def train_step(self, states, actions, yi, mode=None):
        current_Q = self(states, actions)

        critic_loss = F.mse_loss(current_Q, yi)

        self.optimizer.zero_grad()
        if mode == 'global':  # for global critic in DEMADDPG
